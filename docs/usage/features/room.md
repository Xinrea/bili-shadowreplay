...
> [!WARNING]
> B 站、抖音直播间需要先配置对应平台账号；斗鱼、虎牙、快手、TikTok、Twitch 和 YouTube 可直接添加。Twitch 使用公开接口和匿名聊天连接，不需要配置账号。
...
直播间号通常是直播间网页地址尾部的数字，例如 `https://live.bilibili.com/123456` 中的 `123456`，或是 `https://live.douyin.com/123456` 中的 `123456`。斗鱼支持输入数字房间号或 `https://www.douyu.com/<房间号或短名>` 链接。
...
Twitch 直播间请输入频道登录名（例如 `ninja`、`@ninja`），也可以直接粘贴 `https://www.twitch.tv/ninja`。Twitch 弹幕通过公开 IRC 连接获取。
...
在浏览器中观看直播时，替换地址栏中直播间地址中的 `https://` 为 `bsr://` 即可快速唤起 BSR 添加直播间，例如斗鱼地址可写成 `bsr://www.douyu.com/123456`。
