...
fn pack(buffer: &[u8]) -> Result<BilibiliPackCtx, DanmakuStreamError> {
    let data = buffer
        .pread_with(0, scroll::BE)
        .map_err(|e: scroll::Error| DanmakuStreamError::PackError { err: e.to_string() })?;
...
pub fn build_pack(buf: &[u8]) -> Result<Vec<String>, DanmakuStreamError> {
    let ctx = pack(buf)?;
    let msgs = decode(ctx)?;
...
fn get_hot_count(body: &[u8]) -> Result<u32, DanmakuStreamError> {
    let count = body
        .pread_with::<PackHotCount>(0, scroll::BE)
        .map_err(|e| DanmakuStreamError::PackError { err: e.to_string() })?
        .count;

    Ok(count)
}

fn zlib_decode(body: &[u8]) -> Result<(BilibiliPackHeader, Vec<u8>), DanmakuStreamError> {
    let mut buf = vec![];
...
fn decode(ctx: BilibiliPackCtx) -> Result<Vec<String>, DanmakuStreamError> {
    ...
    let msgs = match header.ver {
        ...
        x => return Err(DanmakuStreamError::UnsupportProto { proto: x }),
    };

    Ok(msgs)
}

fn split_msgs(buf: Vec<u8>, header: BilibiliPackHeader) -> Result<Vec<String>, DanmakuStreamError> {
    let mut buf = buf;
    ...
    while offset != buf_len as u32 {
        let ctx = pack(&buf).map_err(|e| DanmakuStreamError::PackError { err: e.to_string() })?;
...
fn brotli_decode(body: &[u8]) -> Result<(BilibiliPackHeader, Vec<u8>), DanmakuStreamError> {
