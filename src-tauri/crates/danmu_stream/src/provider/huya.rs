...
impl HuyaDanmu {
    async fn connect_and_handle(
        ...
        *self.write.write().await = Some(write);
        ...
        tokio::select! {
            v = Self::send_heartbeat_packets(heartbeat_writer, heartbeat_frame) => v,
        ...
        while let Ok(Some(msg)) = read.try_next().await {
            ...
            match command.cmd_type {
                WS_CMD_REGISTER_RSP => match decode_register_rsp(&command.data) {
                    ...
                    Ok(Some(code)) => warn!(
                        "Huya danmaku register rejected ({}), room_id: {}",
                        code, room_id
                    ),
...
fn parse_channel_ids(page: &str) -> Option<RoomChannelIds> {
    let topsid = Regex::new(r#""lChannelId"\s*:\s*(\d+)"#)
        .ok()?
        .captures(page)?
        .get(1)?
        .as_str()
        .parse()
        .ok()?;
    let subsid = Regex::new(r#""lSubChannelId"\s*:\s*(\d+)"#)
        .ok()?
        .captures(page)?
        .get(1)?
        .as_str()
        .parse()
        .ok()?;
    let yyuid = Regex::new(r#""lUid"\s*:\s*(\d+)"#)
        .ok()?
        .captures(page)?
        .get(1)?
        .as_str()
...
mod tests {
    ...
    fn channel_ids_missing_when_offline() {
        assert!(parse_channel_ids(r#"{"roomProfile":{"lUid":431653844}}"#).is_none());
    }
}
