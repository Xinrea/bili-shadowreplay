...
impl TikTokRecorder {
    ...
    async fn check_status(&self) -> bool {
        log::info!("[{}] Check status", self.room_id);
        ...
        match api::get_room_info(&self.client, &self.account, &self.room_id).await {
            Ok(room_info) => {
                ...
                *self.platform_live_id.write().await = Utc::now().timestamp().to_string();
                ...
                let new_stream =
                    api::get_stream_url(&self.client, &self.account, &self.room_id).await;
    ...
    async fn update_entries(&self, live_id: &str) -> Result<(), RecorderError> {
        ...
        *self.live_id.write().await = live_id.to_string();
        ...
        let stream_url = rtmp_url.clone().ok_or(RecorderError::NoStreamAvailable)?;
