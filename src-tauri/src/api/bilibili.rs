...
use crate::recorder::bilibili::stream::BiliStream;
use crate::recorder::stream::StreamType;
...
impl BiliClient {
    pub fn new() -> Result<BiliClient, ApiError> {
        ...
        if let Ok(client) = Client::builder().timeout(Duration::from_secs(10)).build() {
            ...
            Err(ApiError::InitClientError)
        }
    }

    pub async fn fetch_webid(&self, account: &AccountRow) -> Result<String, ApiError> {
        ...
        let re =
            Regex::new(r#"<script id="__RENDER_DATA__" type="application/json">(.+?)</script>"#)
                .unwrap();
        let cap = re.captures(&content).ok_or(ApiError::InvalidValue)?;
        let str = cap.get(1).ok_or(ApiError::InvalidValue)?.as_str();
        // str need url decode
        let json_str = urlencoding::decode(str).map_err(|_| ApiError::InvalidValue)?; // url decode
        let json: serde_json::Value = serde_json::from_str(&json_str).unwrap();
        let webid = json["access_id"].as_str().ok_or(ApiError::InvalidValue)?;
    ...
    pub async fn get_qr_status(&self, qrcode_key: &str) -> Result<QrStatus, ApiError> {
        let res: serde_json::Value = self
            .client
            .get(format!(
    ...
    pub async fn logout(&self, account: &AccountRow) -> Result<(), ApiError> {
    ...
    pub async fn get_room_info(
        ...
        let res: serde_json::Value = self
            ...
            .json()
            .await?;
        let code = res["code"].as_u64().ok_or(ApiError::InvalidValue)?;
        if code != 0 {
            return Err(ApiError::InvalidCode);
        }
        ...
        let room_keyframe_url = res["data"]["keyframe"]
            ...
            .ok_or(ApiError::InvalidValue)?
            .to_string();
        let user_id = res["data"]["uid"].as_u64().ok_or(ApiError::InvalidValue)?;
        let live_status = res["data"]["live_status"]
            .as_u64()
            .ok_or(ApiError::InvalidValue)? as u8;
        Ok(RoomInfo {
            room_id,
            room_title,
            room_cover_url,
    ...
    async fn get_stream(&self, format: &Format) -> Result<BiliStream, ApiError> {
    ...
    pub async fn get_index_content(&self, url: &String) -> Result<String, ApiError> {
        Ok(self
            .client
            .get(url.to_owned())
            .headers(self.headers.clone())
    ...
    pub async fn download_ts(&self, url: &str, file_path: &str) -> Result<u64, ApiError> {
        let res = self
            .client
            .get(url)
            .headers(self.headers.clone())
    ...
    // Method from js code
    pub async fn get_sign(&self, mut parameters: Value) -> Result<String, ApiError> {
        let table = vec![
    ...
    async fn preupload_video(
        ...
        account: &AccountRow,
        video_file: &Path,
    ) -> Result<PreuploadResponse, ApiError> {
        let mut headers = self.headers.clone();
        headers.insert("cookie", account.cookies.parse().unwrap());
        let url = format!(
    ...
    async fn post_video_meta(
        ...
        video_file: &Path,
    ) -> Result<PostVideoMetaResponse, ApiError> {
        let url = format!(
    ...
    pub async fn submit_video(
        ...
        video: &profile::Video,
    ) -> Result<VideoSubmitData, ApiError> {
        let mut headers = self.headers.clone();
        headers.insert("cookie", account.cookies.parse().unwrap());
        let url = format!(
    ...
    pub async fn get_video_typelist(
        &self,
        account: &AccountRow,
    ) -> Result<Vec<response::Typelist>, ApiError> {
