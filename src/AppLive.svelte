<script lang="ts">
    ...
    $: video_src = video
        ? convertFileSrc(config.output + "/" + video.name)
        : "";
    ...
    async function generate_clip() {
        ...
        appWindow.setTitle(
            `[${room_id}][${ts}]${archive.title} - 切片生成成功`,
        );
        ...
        >
            <div>
                ...
                >
                    <div
                        id="capture"
                        class="cover-wrap relative cursor-pointer"
                    >
