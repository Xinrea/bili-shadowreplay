<script lang="ts">
  ...
  function toggleAutoRecord(roomId: number, platform: string) {
    const key = `${platform}-${roomId}`;
    autoRecordStates.set(key, !autoRecordStates.get(key));
    autoRecordStates = autoRecordStates; // Trigger reactivity
  }

  // Function to get auto-record state
  function getAutoRecordState(roomId: number, platform: string): boolean {
    const key = `${platform}-${roomId}`;
    return autoRecordStates.get(key) || false;
  }
</script>
      ...
      {#each filteredRecorders as room (room.room_id)}
        ...
        >
          <div class="relative">
            ...
            />
            <div
              class={"absolute top-2 left-2 p-1.5 rounded-full flex items-center justify-center " +
            ...
            <div
              class={"absolute top-2 left-12 p-1.5 px-2 rounded-full text-white text-xs flex items-center justify-center " +
                (room.live_status ? "bg-green-500" : "bg-gray-700")}
            >
              <span>{room.live_status ? "直播中" : "未直播"}</span>
            </div>
            {#if getAutoRecordState(room.room_id, room.platform)}
              <div
                class="absolute top-2 left-[5.5rem] p-1.5 rounded-full bg-gray-700/90"
              >
            ...
            <Dropdown class="whitespace-nowrap">
              ...
              >
                ...
                <label class="toggle-switch ml-1">
                  <input
                    type="checkbox"
                    checked={getAutoRecordState(room.room_id, room.platform)}
                  />
